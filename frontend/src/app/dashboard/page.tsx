...
import { Plus, Trash2, Map, Clock, RefreshCw, Crosshair, Truck, X, Send, Package, Brain } from "lucide-react";
...
const RebalanceTab = dynamic(() => import("@/components/RebalanceTab"), { ssr: false });

type TabKey = "map" | "inventory" | "rebalance";

const TABS: { key: TabKey; label: string; iconName: string }[] = [
    { key: "map", label: "Map & Dispatch", iconName: "map" },
    { key: "inventory", label: "Inventory", iconName: "package" },
    { key: "rebalance", label: "GNN Rebalance", iconName: "brain" },
];
...
export default function Dashboard() {
    ...
    const loadData = useCallback(async () => {
        setLoading(true);
        try {
            const [cData, covData] = await Promise.all([fetchCentres(), fetchCoverage()]);
            setCentres(cData);
            setCoverage(covData);
    ...
    }, [newName, newLat, newLon, loadData]);
    ...
    const handleDispatch = useCallback(async () => {
        ...
        try {
            ...
            if (result.error) {
                alert(`Dispatch Error: ${result.error}`);
            }
            if (result.riders && result.riders.length > 0) {
                ...
                alert("No routes could be computed. Ensure order coordinates are within range of your hubs.");
            }
        } catch (e) { console.error(e); }
        setDispatchLoading(false);
    }, [dispatchOrders_, riderCount]);

    // Stable coverage data slice for the active mode
    const coverageForMode = useMemo(() => coverage?.[mode] || {}, [coverage, mode]);

    const isMap = activeTab === "map";

    return (
        <div className="flex h-full w-full">
            {/* Sidebar — only visible on Map tab */}
            {isMap && (
                <div className="w-80 bg-[#121214] border-r border-white/10 flex flex-col shadow-2xl z-20 overflow-y-auto will-change-scroll">
                    ...
                    <div className="p-6 border-b border-white/10 space-y-3">
                        ...
                        <button onClick={() => { setPinMode(!pinMode); if (pinMode) setEtaPopup(null); }}
                            className={`w-full py-2.5 text-sm font-medium rounded-lg flex items-center justify-center gap-2 transition-colors ${
                                pinMode ? 'bg-red-600 hover:bg-red-700' : 'bg-white/5 hover:bg-white/10 border border-white/10'}`}>
                            <Crosshair size={16} />
                        ...
                        <button onClick={() => { setShowDispatch(!showDispatch); if (!showDispatch) { setPinMode(false); setEtaPopup(null); } }}
                            className={`w-full py-2.5 text-sm font-medium rounded-lg flex items-center justify-center gap-2 transition-colors ${
                                showDispatch ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-white/5 hover:bg-white/10 border border-white/10'}`}>
                            <Truck size={16} />
                        ...
                        <div className="p-6 border-b border-white/10 space-y-3">
                            ...
                            <div className="space-y-2">
                                <input value={orderAddr} onChange={e => setOrderAddr(e.target.value)} placeholder="Address text"
                                    className="w-full bg-black/50 border border-white/10 rounded-md px-3 py-2 text-xs outline-none focus:border-emerald-500" />
                                <div className="flex gap-2">
                                    <input value={orderLat} onChange={e => setOrderLat(e.target.value)} placeholder="Lat"
                                        className="w-1/2 bg-black/50 border border-white/10 rounded-md px-3 py-2 text-xs outline-none focus:border-emerald-500" />
                            ...
                            {dispatchOrders_.length > 0 && (
                                <div className="space-y-1.5 max-h-32 overflow-y-auto">
                                    {dispatchOrders_.map((o, i) => (
                                        <div key={o.order_id} className="flex items-center justify-between p-2 bg-white/5 rounded-md text-xs">
                            ...
                            <button onClick={handleDispatch} disabled={dispatchLoading || dispatchOrders_.length === 0}
                                className="w-full py-2.5 text-sm font-bold bg-emerald-600 hover:bg-emerald-700 disabled:opacity-40 rounded-lg flex items-center justify-center gap-2 transition-colors">
                    ...
                    <div className="p-6 border-b border-white/10">
                        ...
                        <form onSubmit={handleAdd} className="flex flex-col gap-3">
                            <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Hub Name" className="bg-black/50 border border-white/10 rounded-md px-3 py-2 text-sm outline-none focus:border-blue-500" />
                            <div className="flex gap-2">
                                <input value={newLat} onChange={e => setNewLat(e.target.value)} placeholder="Lat" className="w-1/2 bg-black/50 border border-white/10 rounded-md px-3 py-2 text-sm outline-none focus:border-blue-500" />
                                <input value={newLon} onChange={e => setNewLon(e.target.value)} placeholder="Lon" className="w-1/2 bg-black/50 border border-white/10 rounded-md px-3 py-2 text-sm outline-none focus:border-blue-500" />
                            </div>
            ...
            <div className="flex-1 flex flex-col relative bg-black">
                {/* Tab Navigation Bar */}
                <div className="h-14 bg-[#121214] border-b border-white/10 flex items-center px-6 gap-1 z-30 shrink-0">
                    {TABS.map(tab => (
                        <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                            ...
                            {TAB_ICONS[tab.iconName]} {tab.label}
                        </button>
                    ))}
                    <div className="flex-1" />
                    <div className="flex items-center gap-2 text-xs text-gray-600">
                        <span className="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_6px_rgba(34,197,94,0.6)]" />
                        {centres.length} hubs active
                    </div>
                </div>

                {/*
                  KEY PERF FIX: Use display:none instead of conditional rendering.
                  This keeps the MapComponent mounted (preserving WebGL context)
                  and avoids expensive re-initialization on tab switch.
                */}
                <div className="flex-1 relative p-4" style={{ display: isMap ? 'block' : 'none' }}>
                    {loading && (
                ...
                <div style={{ display: activeTab === "inventory" ? 'flex' : 'none' }} className="flex-1 min-h-0 overflow-hidden">
                    <InventoryTab centres={centres} />
                </div>

                <div style={{ display: activeTab === "rebalance" ? 'flex' : 'none' }} className="flex-1 min-h-0 overflow-hidden">
                    <RebalanceTab />
                </div>
            </div>
        </div>
